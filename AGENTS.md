...
『ガンダムタクティクス MOBILITY FLEET0079 復刻版』(2001-09-19 発売、Windows 95/98/Me 用)を
Windows 11 上で遊べるようにする **配布可能な非公式互換パッチ** を作成・保守する。
...
- `source_iso/`(インストールディスク)と `source_exe/`(インストール直後のファイル群)は **絶対に改変しない**。
  書き込み・リネーム・削除も禁止。ビルドと検証の原本としてだけ使う。
- 実行検証は `run/GT/` で行う(`source_exe/` の複製。自由に上書きしてよく、壊れたら再コピー)。
...
| パス | 用途 | 書き換え |
|---|---|---|
| `source_iso/` | インストールディスク ISO(原本、git 管理外) | 禁止 |
| `source_exe/` | インストール直後のファイル群(原本、git 管理外) | 禁止 |
| `run/GT/` | 実行検証用ゲームコピー(git 管理外) | 自由 |
...
- 一括ビルド: `python work/build_all.py`(`source_exe/gundam.exe` の SHA256 を照合してから実行される)。
...
- 実機検証: ゲームは GUI アプリ。`run/GT/gundam.exe` を起動し、ウィンドウ(`Gundam Tactics`)の存在、
...
- 配布 ZIP にゲームの著作物を含めない。インストーラー(`install.bat` → `patch_files/install.ps1`)が
  利用者の `gundam.exe` の SHA256 を照合し、未改変の復刻版または既知の旧パッチ版にだけ適用する。
