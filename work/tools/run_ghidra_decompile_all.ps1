param(
    ...
    [string]$ProjectDirectory = (Join-Path $PSScriptRoot '..\analysis\ghidra_all'),
    [string]$ProjectName = 'GundamTacticsAll'
)
...
$out = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..\analysis\decomp_all.c'))
$scriptDirectory = Join-Path $PSScriptRoot '..\analysis\ghidra_scripts'
New-Item -ItemType Directory -Force -Path $ProjectDirectory | Out-Null
$ghidraUserRoot = Join-Path $PSScriptRoot '..\analysis\ghidra_user_all'
...
& $headless $ProjectDirectory $ProjectName `
    ...
    -postScript DecompileAll.java $out `
    -deleteProject
